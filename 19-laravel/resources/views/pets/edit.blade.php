        ...
        <ul>
            ...
            <a href="{{ url('dashboard') }}">
                <svg xmlns="http://www.w3.org/2000/svg" class="size-4" fill="currentColor" viewBox="0 0 256 256">
                    <path d="M104,40H56A16,16,0,0,0,40,56v48a16,16,0,0,0,16,16h48a16,16,0,0,0,16-16V56A16,16,0,0,0,104,40Zm0,64H56V56h48v48Zm96-64H152A16,16,0,0,0,136,56v48a16,16,0,0,0,16,16h48a16,16,0,0,0,16-16V56A16,16,0,0,0,200,40Zm0,64H152V56h48v48Zm-96,32H56a16,16,0,0,0-16,16v48a16,16,0,0,0,16,16h48a16,16,0,0,0,16-16V152A16,16,0,0,0,104,136Zm0,64H56V152h48v48Zm96-64H152a16,16,0,0,0-16,16v48a16,16,0,0,0,16,16h48a16,16,0,0,0,16-16V152A16,16,0,0,0,200,136Zm0,64H152V152h48v48Z"></path>
                </svg>
                Dashboard
            </a>
            </li>
        ...
        <div class="card text-white md:w-[720px] w-[320px] bg-black/20 p-4 mb-4 rounded">
            <form method="POST" action="{{ url('pets/'.$pet->id) }}" class="flex flex-col md:flex-row gap-4 mt-4" enctype="multipart/form-data">
                ...
                <div class="w-full md:w-[320px]">
                    ...
                    <input type="text" class="input bg-[#0009] outline-0" name="name" placeholder="Luna" value="{{ old('name', $pet->name) }}">
                    @error('name')
                        <small class="badge badge-error w-full mt-1 text-xs py-4">{{ $message }}</small>
                    @enderror
                    ...
                    <select name="kind" class="select bg-[#0009] outline-0">
                        ...
                        <option value="dog" @if(old('kind', strtolower($pet->kind)) == 'dog') selected @endif>Dog</option>
                        <option value="cat" @if(old('kind', strtolower($pet->kind)) == 'cat') selected @endif>Cat</option>
                        <option value="pig" @if(old('kind', strtolower($pet->kind)) == 'pig') selected @endif>Pig</option>
                        <option value="bird" @if(old('kind', strtolower($pet->kind)) == 'bird') selected @endif>Bird</option>
                    </select>
                    @error('kind')
                        <small class="badge badge-error w-full mt-1 text-xs py-4">{{ $message }}</small>
                    @enderror
                    <label class="label text-white">Weight:</label>
                    <input type="text" class="input bg-[#0009] outline-0" name="weight" placeholder="5.5" value="{{ old('weight', $pet->weight) }}">
                    @error('weight')
                        <small class="badge badge-error w-full mt-1 text-xs py-4">{{ $message }}</small>
                    @enderror
                ...
                <div class="w-full md:w-[320px]">
                    ...
                    <textarea name="description" class="textarea bg-[#0009] outline-0 w-full min-h-24" placeholder="Friendly pet">{{ old('description', $pet->description) }}</textarea>
                    @error('description')
                        <small class="badge badge-error w-full mt-1 text-xs py-4">{{ $message }}</small>
                    @enderror
